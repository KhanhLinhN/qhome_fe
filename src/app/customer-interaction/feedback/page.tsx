...
import { useFeedbacks } from '@/src/hooks/useTableFeedback';
...
export default function Home() {
  ...
  const headers = [t('unitCode') || 'Mã căn hộ', t('feedbackTitle'), t('residentName'), t('dateCreated'), t('status'), t('action')];
  ...
  // Fetch unit and resident info for all feedbacks
  useEffect(() => {
    const fetchUnitAndResidentInfo = async () => {
      if (!allFeedbacksList || allFeedbacksList.length === 0) return;
      ...
      // Build combined map - use feedback id as key for unique mapping
      allFeedbacksList.forEach(item => {
        newMap.set(item.id, {
  ...
  const { filteredTableData, filteredTotalPages } = useMemo(() => {
    // Always use allFeedbacksList - all filtering is done in frontend
    const sourceData = allFeedbacksList || [];
  ...
  }, [allFeedbacksList, activeStatus, filters.search, filters.dateFrom, filters.dateTo, pageNo]);
  ...
  return (
    <div className="lg:col-span-1 space-y-6">
      <div className="max-w-screen overflow-x-hidden ">
          <h1 className="text-2xl font-semibold text-[#02542D] mb-4">{t('feedbackList')}</h1>
