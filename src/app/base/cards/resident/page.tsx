...
import {
  decideResidentCardRegistration,
...
export default function ResidentCardAdminPage() {
  ...
  const loadDetail = useCallback(
    async (id: string) => {
      ...
      setDetailError(null);
      try {
        const data = await fetchResidentCardRegistration(id);
        setSelected(data);
  ...
  const handleDecision = async (decision: 'APPROVE' | 'REJECT') => {
    ...
    setSubmitting(true);
    try {
      const updated = await decideResidentCardRegistration(selectedId, {
        decision,
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3 text-sm">
                ...
                <DetailRow
                  ...
                  value={selected.requestType ? (requestTypeConfig[selected.requestType] ?? selected.requestType) : '—'}
                />
